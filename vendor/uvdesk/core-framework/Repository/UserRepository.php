...
class UserRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getAllCustomer(ParameterBag $obj = null, ContainerInterface $container)
    {
        $json = array();
        $qb = $this->getEntityManager()->createQueryBuilder();
    ...
    {
        $query = $this->getEntityManager()->createQueryBuilder();
        $query->select("user.id,user.email,CONCAT(user.firstName,' ', COALESCE(user.lastName,'')) AS name,user.firstName,user.lastName,user.isEnabled,userInstance.contactNumber,userInstance.profileImagePath,userInstance.profileImagePath as smallThumbnail,userInstance.isActive, userInstance.isVerified,user.programme,user.faculty,user.department,user.matricNumber")->from('UVDeskCoreFrameworkBundle:User', 'user')
        ->leftJoin('user.userInstance', 'userInstance')
        ->andwhere('userInstance.supportRole = :roles')
        ->andwhere('user.email = :email')
        ->setParameter('roles', UserRepository::CUSTOMER_ROLE)
